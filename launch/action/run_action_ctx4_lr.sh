...
export NG_RUN_ROOT_DIR="results/large_zelda_ctx4_lr3e5"
export WANDB_RUN_GROUP="large_zelda_ctx4_lr3e5"
...
echo "ACTION ctx4 LR3e-5: lower LR (3e-5 vs 1e-4) + variance band + delta -- is the collapse an optimization instability?"
nvidia-smi -L
torchrun --standalone --nproc_per_node=4 scripts/train_latent_actions.py \
  --config configs/latent_actions_large_lr3e5.yaml --training_config configs/training_ddp.yaml
