...
echo "LARGE tokenizer (embed512) + perceptual, 4xA100-80GB on prod (reliable) + 60min NCCL timeout fix"
nvidia-smi -L
torchrun --standalone --nproc_per_node=4 scripts/train_video_tokenizer.py \
  --config configs/video_tokenizer_large.yaml --training_config configs/training_ddp.yaml
